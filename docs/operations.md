...
`cobalt-tool-watch-mark` is centered on cli tools work in Python. The technical target is: Package a Python local lab for watch analysis with windowed input fixtures, late-data behavior checks, and documented operating limits.
...
The score is deterministic. The current threshold is 164. Risk has a penalty of 4, latency has a penalty of 4, and weight contributes a bonus of 6.
